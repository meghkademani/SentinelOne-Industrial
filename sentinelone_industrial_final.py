...
def create_log_file():
    ...
    # Upgrade the original 5-column CSV once, preserving all
    # existing records and creating a backup first.
    try:
        ...
        if "Event Reason" not in header:

            backup_file = LOG_FILE.replace(
                ".csv",
                "_backup_before_stage3.csv"
            )
            ...
            print(
                "Safety log upgraded successfully."
            )
            print(
        ...
        print(
            f"WARNING: Could not upgrade safety log: {error}"
        )
...
def get_event_reason(
    ...
    # Zone intrusion gets highest priority because it is an
    # independent visual safety condition.
    if zone_intrusion:
        return "DANGER ZONE INTRUSION"

    if gas > GAS_DANGER and distance <= DISTANCE_DANGER:
        return "GAS + PROXIMITY"
    ...
    if gas > GAS_WARNING and distance <= DISTANCE_WARNING:
        return "GAS + PROXIMITY WARNING"
...
# =========================================================
# STAGE 5 - EVENT HISTORY
# =========================================================

def load_recent_events(limit=4):
    """Load the latest valid safety events from the CSV log."""
    ...
    try:
        with open(
            ...
            for row in reader:
                ...
                status_value = row.get("Safety Status", "")
                reason_value = row.get("Event Reason", "")

                if timestamp and status_value:
                    history.append(
                        (
                            timestamp,
                            status_value,
                            reason_value or "NORMAL"
                        )
                    )
...
event_history = load_recent_events(limit=4)
...
def calculate_safety(gas, distance, zone_intrusion=False):

    # A worker entering the configured danger zone is an immediate
    # DANGER condition, independent of gas/distance readings.
    if zone_intrusion:
        return "DANGER"
...
try:
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # Use a point just above the bottom of the YOLO box.
            # This is more reliable when a worker's bounding box is
            # clipped by the bottom edge of the webcam frame.
            foot_x = x + (w // 2)
            foot_y = y + h - 5
        ...
        # =====================================================
        # EVENT REASON - STAGE 3
        # =====================================================

        event_reason = get_event_reason(
            gas_value,
            distance,
            zone_intrusion
        )
        ...
        # =====================================================
        # LOG STATUS / REASON CHANGES - STAGE 3
        # =====================================================
        # Avoid writing the same event every video frame.
        # A new row is written when either the safety status or
        # its cause changes.
        ...
        if previous_status is None:
            ...
            previous_reason = event_reason

        elif current_event != previous_event:
        ...
        # =====================================================
        # STAGE 5 - COMPACT RECENT SAFETY EVENTS PANEL
        # =====================================================

        # Kept between the alert banner and the camera danger-zone
        # overlay so the event history does not hide worker labels.
        history_x1 = 320
        history_y1 = 122
        history_x2 = width - 15
        history_y2 = 250
        ...
        recent_events = list(event_history)[::-1]
        ...
        if not recent_events:
            ...
            for timestamp, event_status, reason in recent_events:

                if event_status == "DANGER":
                    history_color = RED
                elif event_status == "WARNING":
                    history_color = ORANGE
                else:
                    history_color = GREEN

                # CSV timestamps are YYYY-MM-DD HH:MM:SS.
                # Display only HH:MM:SS for a cleaner dashboard.
                display_time = timestamp[-8:] if len(timestamp) >= 8 else timestamp

                short_reason = reason.replace("_", " ")

                if len(short_reason) > 27:
                    short_reason = short_reason[:24] + "..."

                history_line = (
                    f"{display_time}  {event_status}  |  {short_reason}"
                )
                ...
                if row_y > history_y2 - 8:
                    break


        # =====================================================
        # STAGE 4 - REAL-TIME ALERT BANNER
        # =====================================================

        alert_height = 55
        alert_y1 = 58
        alert_y2 = alert_y1 + alert_height

        if status == "DANGER":
            alert_text = f"DANGER: {event_reason}"
            alert_color = RED

        elif status == "WARNING":
            alert_text = f"WARNING: {event_reason}"
            alert_color = ORANGE
        ...
        print(
            ...
            f"Zone: {'INTRUSION' if zone_intrusion else 'CLEAR'} | "
            f"Status: {status} | "
